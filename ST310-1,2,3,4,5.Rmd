...
drug_data_copy <- drug_data

# Convert specified variables to factors in the copied data
drug_data_copy[drug_vars] <- lapply(drug_data_copy[drug_vars], function(x) {
  factor(x, levels = paste0("CL", 0:6))
})

# Split the copied data into training and testing sets
set.seed(38036)
train_indices <- createDataPartition(drug_data_copy$heroin, p = 0.8, list = FALSE)

train_data <- drug_data_copy[train_indices, ]
test_data  <- drug_data_copy[-train_indices, ]
```
...
```{r}
X_train <- train_data2[, setdiff(names(train_data2), "heroin_level")]
X_test  <- test_data2[, setdiff(names(test_data2), "heroin_level")]
